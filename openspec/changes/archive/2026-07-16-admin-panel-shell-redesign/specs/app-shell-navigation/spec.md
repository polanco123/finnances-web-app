...
Authenticated application shell (sidebar + topbar) providing shared, consistent navigation and utility controls across all authenticated routes (`/`, `/movimientos`, `/diversion`), replacing per-page chrome duplication.

## ADDED Requirements

### Requirement: Functional sidebar navigation links
The sidebar SHALL render three functional navigation links — Dashboard (`/`), Movimientos (`/movimientos`), Diversión (`/diversion`) — that navigate the application to the corresponding route when activated.
...
#### Scenario: All three functional links are present
- GIVEN the shell is rendered
- WHEN the sidebar is inspected
- THEN it SHALL contain exactly three functional links labeled for Dashboard, Movimientos, and Diversión, targeting `/`, `/movimientos`, and `/diversion` respectively

### Requirement: Placeholder navigation entries for future phases
The sidebar SHALL also render four placeholder entries — Cuentas, Categorías, Reportes, Configuración — for capabilities not implemented this change. Activating a placeholder entry MUST NOT produce a broken/404 experience or an unhandled error. The exact placeholder treatment (disabled control, inert stub link, "próximamente" page, etc.) is a design-phase decision, not specified here.

#### Scenario: Activating a placeholder entry does not break the app
- GIVEN the shell is rendered with the sidebar visible
- WHEN the user activates the "Cuentas" placeholder entry
...
- GIVEN the shell is rendered
- WHEN the sidebar is inspected
- THEN the four placeholder entries (Cuentas, Categorías, Reportes, Configuración) SHALL be present
- AND they SHALL be visually distinguishable from the three functional links (e.g. a disabled/muted visual treatment), so the user can tell they are not yet available
