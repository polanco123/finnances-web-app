...
> **Bookkeeping note (2026-08-14)**: E.3's original plan (querying `deuda_pago` by a shared month-start `periodo`) was stale — it predated the per-account `periodo` rework (each account's `periodo` is its own real due date via `dia_pago`, not a shared month key). Implemented instead by reusing `fetchLatestPagosPorCuentas(cuentaIds)` from `components/deudas/deudas-service.ts`, the same "latest record per account" resolution `/deudas` itself uses — this keeps the widget's monto data reconciled with `/deudas` by construction instead of inventing a second matching strategy. `formatMonto` was likewise replaced by reusing the existing `formatMoney` from `components/patrimonio/patrimonio-format.ts` rather than a new local helper.

- [x] E.1 Modify `components/patrimonio/patrimonio-service.ts`'s `VencimientoCuenta` interface — add `montoPlaneado?: number`, `montoPagado?: number | null`, `pagado?: boolean` (all optional; present only when a `deuda_pago` row exists for the account).
- [x] E.2 Modify `fetchProximosVencimientos()` in the same file — add `.eq('tipo', 'deuda')` to the existing `cuenta` query (alongside `activa=true` and `dia_pago IS NOT NULL`); return early (`return vencimientos`) when the vencimientos list is empty, skipping the second query entirely.
- [x] E.3 In the same function, when vencimientos is non-empty: call `fetchLatestPagosPorCuentas(vencimientos.map(v => v.id))` (imported from `@/components/deudas/deudas-service`), merge into the result via the returned `Map` keyed by `cuentaId`, populating `montoPlaneado`/`pagado`/`montoPagado` only for matched accounts.
- [x] E.4 Modify `components/patrimonio/patrimonio-vencimientos.tsx` — reuse `formatMoney` from `./patrimonio-format` and render a `patrimonio-vencimiento-strip__monto` span only when `v.montoPlaneado !== undefined`, showing `v.montoPagado` when `v.pagado`, else `v.montoPlaneado`; wrap the monto span + existing `diasRestantes` label in a new `patrimonio-vencimiento-strip__right` container.
- [x] E.5 Modify `components/patrimonio/patrimonio-vencimientos.css` — added `.patrimonio-vencimiento-strip__right` (flex row, `gap: 12px`), `.patrimonio-vencimiento-strip__monto` (`color: var(--ink-dim)`), `.patrimonio-vencimiento-strip__monto--pagado` (`color: var(--up)`). Verified via `grep -n -- "--theme-"` against this file — zero matches.

## Phase F: Sidebar

> **Bookkeeping note (2026-08-14)**: done earlier, independently, with a functionally equivalent icon — `sidebar.tsx` already has `{ href: '/deudas', label: 'Deudas', icon: MinusCircle }` inserted before the `comingSoon: true` Configuración entry. `MinusCircle` instead of the originally planned `CreditCard`; no functional gap.

- [x] F.1 Modify `components/app-shell/sidebar.tsx` — add a `/deudas` entry to `NAV_ITEMS` immediately after the Patrimonio entry (`/reportes`) and before the `comingSoon: true` Configuración entry — Configuración must remain last.
...
- [x] G.10 `npm run lint` (project files clean; unrelated pre-existing errors only in stale `dist/` build output, not linted normally) and `npm run build` both pass with zero errors (verified 2026-08-14).
