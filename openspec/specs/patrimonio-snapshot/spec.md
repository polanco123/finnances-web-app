...
#### Scenario: Monetary columns computed from saldo_calculado, not saldo_real
...
- THEN the computation SHALL use `SUM(cuenta.saldo_calculado)` exclusively
- AND `saldo_real` SHALL NOT be read for this computation

### Requirement: Snapshot values derived from active accounts

The system SHALL compute, scoped to `activa = true` accounts: `patrimonio_neto = SUM(saldo_calculado)`; `patrimonio_disponible = patrimonio_neto - SUM(saldo_calculado WHERE es_fondo_retiro = true)`; `total_activos = SUM(saldo_calculado) WHERE saldo_calculado > 0`; `total_deudas = SUM(saldo_calculado) WHERE saldo_calculado < 0`, stored as its positive magnitude.

#### Scenario: Standard computation

- GIVEN active accounts with saldo_calculado values 1000, 500, and -200, where the 500 account is flagged `es_fondo_retiro = true`
...
- GIVEN an `activa = false` account with a nonzero `saldo_calculado`
- WHEN the daily snapshot computes
- THEN that account's `saldo_calculado` SHALL NOT contribute to any of the four snapshot columns
